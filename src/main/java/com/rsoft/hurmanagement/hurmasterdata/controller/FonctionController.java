...
import com.rsoft.hurmanagement.hurmasterdata.service.FonctionService;
...
public class FonctionController {
    ...
    @GetMapping
    public ResponseEntity<Page<FonctionDTO>> findAll(
    ...
    @PostMapping
    public ResponseEntity<FonctionDTO> create(
            @Valid @RequestBody FonctionCreateDTO dto,
            @RequestHeader(value = "X-Username", defaultValue = "system") String username) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(dto, username));
    }
    
    @PutMapping("/{id}")
    public ResponseEntity<FonctionDTO> update(
            @PathVariable Long id,
            @Valid @RequestBody FonctionUpdateDTO dto,
            @RequestHeader(value = "X-Username", defaultValue = "system") String username) {
